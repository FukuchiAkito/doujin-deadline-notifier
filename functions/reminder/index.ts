...
async function privateVapidKey() {
  ...
  if (!parameterName)
    throw new Error("VAPID_PRIVATE_KEY_PARAMETER is required");
  const result = await ssm.send(
    new GetParameterCommand({ Name: parameterName, WithDecryption: true }),
  );
  if (!result.Parameter?.Value)
    throw new Error("VAPID private key was not found");
  return result.Parameter.Value;
}

export async function handler() {
  const [deadlinesResult, vapidPrivateKey] = await Promise.all([
    dynamo.send(new ScanCommand({ TableName: process.env.DEADLINES_TABLE })),
    privateVapidKey(),
  ]);
  ...
  for (const deadline of (deadlinesResult.Items ?? []) as Deadline[]) {
    ...
    const watches = await dynamo.send(
      new QueryCommand({
        TableName: userDataTable,
        IndexName: "DeadlineIndex",
        KeyConditionExpression: "gsi1pk = :pk",
        ExpressionAttributeValues: { ":pk": `DEADLINE#${deadline.id}` },
      }),
    );
    for (const watch of watches.Items ?? []) {
      const item = watch as Watch & {
        pk: string;
        sk: string;
        notifiedDays?: Record<string, string>;
      };
      if (
        !item.notifyEnabled ||
        item.progress === "submitted" ||
        item.notifiedDays?.[String(days)]
      )
        continue;
      ...
      await Promise.all(
        (subscriptions.Items ?? []).map(async (subscription) => {
          await webpush.sendNotification(
            ...
            JSON.stringify({
              title: `${deadline.eventName}まであと${days}日`,
              body: `${deadline.label}: ${new Intl.DateTimeFormat("ja-JP", { timeZone: "Asia/Tokyo", month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit" }).format(new Date(deadline.deadlineAt))}`,
              url: "/",
            }),
          );
          sent += 1;
      ...
      if (!item.notifiedDays) {
        await dynamo.send(
          new UpdateCommand({
            ...
            UpdateExpression: "SET notifiedDays = :empty",
      ...
      await dynamo.send(
        new UpdateCommand({
          TableName: userDataTable,
          Key: { pk: item.pk, sk: item.sk },
          UpdateExpression: "SET notifiedDays.#days = :sentAt",
