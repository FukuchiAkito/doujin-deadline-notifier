...
export function DeadlineDashboard({
  ...
  async function addWatch(deadlineId: string) {
    try {
      ...
      setWatches((current) => [
        ...current.filter((watch) => watch.deadlineId !== deadlineId),
        result.data,
      ]);
    } catch (error) {
      setMessage(
  ...
  async function updateWatch(deadlineId: string, values: Partial<Watch>) {
    try {
      const result = await requestJson<ApiResponse<Watch>>(
        `/api/v1/watches/${encodeURIComponent(deadlineId)}`,
        {
          method: "PATCH",
      ...
      setWatches((current) =>
        current.map((watch) =>
          watch.deadlineId === deadlineId ? result.data : watch,
        ),
      );
    } catch (error) {
      setMessage(
        error instanceof Error ? error.message : "進捗を更新できませんでした",
      );
    }
  }
  ...
  async function removeWatch(deadlineId: string) {
    try {
      ...
      setMessage(
        error instanceof Error ? error.message : "監視を解除できませんでした",
      );
    }
  }
  ...
  async function enablePush() {
    ...
    try {
      ...
      setMessage(
        error instanceof Error ? error.message : "通知を有効にできませんでした",
      );
    }
  }
  ...
  return (
    <main className="mx-auto flex w-full max-w-5xl flex-1 flex-col gap-8 px-5 py-10 sm:px-8">
      <header className="flex flex-col gap-5 rounded-3xl bg-[var(--ink)] p-7 text-white shadow-lg sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="mb-2 text-sm font-medium tracking-[0.16em] text-orange-200">
            DOUJIN DEADLINE NOTIFIER
          </p>
          <h1 className="text-3xl font-bold tracking-tight">
            入稿締切を、見落とさない。
          </h1>
          <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">
            オレンジ工房の公開締切情報を確認し、作業の進捗と通知をこのブラウザで管理します。
          </p>
        </div>
        <button
          ...
          type="button"
          disabled={!vapidPublicKey || isPushEnabled}
        >
      ...
      <section className="grid gap-4 sm:grid-cols-3">
        ...
        <div className="summary-card">
          <span>入稿済み</span>
          <strong>
            {watches.filter((watch) => watch.progress === "submitted").length}
          </strong>
          <small>進捗を更新できます</small>
        </div>
        <div className="summary-card">
          <span>通知</span>
          <strong>{isPushEnabled ? "ON" : "OFF"}</strong>
          <small>7・3・1日前</small>
        </div>
      </section>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
        <div className="mb-5 flex flex-wrap items-baseline justify-between gap-2">
          <div>
            <p className="eyebrow">OFFICIAL DEADLINES</p>
          ...
          <a
            ...
            href="https://www.orangekoubou.com/schedule/schedule_event.php"
            target="_blank"
            rel="noreferrer"
          >
            公式ページを確認
        ...
        {isLoading ? (
          <p className="text-sm text-slate-500">締切情報を読み込み中です…</p>
        ) : (
                ...
                <article className="deadline-card" key={deadline.id}>
                  <div className="min-w-0 flex-1">
                    <div className="mb-2 flex flex-wrap gap-2">
                      <span className="tag">{deadline.eventDate}</span>
                      <span
                        className={remaining <= 3 ? "tag tag-urgent" : "tag"}
                      >
                        {remaining >= 0 ? `あと${remaining}日` : "締切済み"}
                      </span>
                    </div>
                    <h3>{deadline.eventName}</h3>
                    <p className="mt-1 text-sm text-slate-600">
                      {deadline.label}
                    </p>
                    {deadline.deadlineCondition && (
                      <p className="mt-1 text-xs text-slate-500">
                        条件: {deadline.deadlineCondition}
                      </p>
                    )}
                    {deadline.deliveryLabel && (
                      <p className="mt-1 text-xs text-slate-500">
                        {deadline.deliveryLabel}
                      </p>
                    )}
                  ...
                  <div className="flex shrink-0 flex-col items-start gap-3 sm:items-end">
                    ...
                    {!watch ? (
                      <button
                        ...
                        type="button"
                        onClick={() => void addWatch(deadline.id)}
                      >
                        監視する
                      </button>
                    ) : (
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <button
                          ...
                          type="button"
                          onClick={() => void removeWatch(deadline.id)}
                        >
                          解除
                        </button>
                      </div>
      ...
      <p className="pb-6 text-xs leading-5 text-slate-500">
        締切は公式情報を定期確認した参考情報です。部数、ページ数、加工、納品方法などで変動するため、入稿前に必ず公式ページを確認してください。
      </p>
    </main>
  );
}
